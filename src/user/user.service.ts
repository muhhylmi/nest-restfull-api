...
export class UserService {
  ...
  async login(request: LoginUserRequest): Promise<UserResponse> {
    ...
    const loginRequest = await this.validationService.validate(
    ...
    if (!user) {
      throw new HttpException('Credential is invalid', 401);
    }
  ...
  async update(user: User, request: UpdateUserRequest): Promise<UserResponse> {
    this.logger.info(`User.Service.update (${JSON.stringify(request)})`);
